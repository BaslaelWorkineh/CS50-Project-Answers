...
void substitute(); // for further substitute
void alpha(char pos, string key);
...
void substitute(string key)
{
    ...
    for (int i = 0; i < strlen(p); i++)
    {
        if (isalpha(p[i])) // checking p[i] is alphabet
        {
            ...
            if (islower(p[i]))
            {
                alpha(tolower(x), key); // passing p[i] in lower-case
            }
            else
            {
                alpha(toupper(x), key); // passing p[i] in UPPER-CASE
            }
...
void alpha(char pos, string key) // passing p[i] and upper/lower(alpha array)
{
